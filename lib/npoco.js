    ...
    renderCode:function(options){
        ...
        schemaReader.readSchema(dbConfig,function(err,tables){
            ...
            if(dbConfig.generate3TierArchitecture) {
                ...
                if(!dbConfig.generateModelWithSingleFile) {
                    tables.forEach(function (m) {


                        var dalClass = renderEngine.renderView('default/single_models.ejs', {dbConfig: dbConfig, tbl: m});
                        var outDir = path.join(dbConfig.outPath, "NPoco.Model");
